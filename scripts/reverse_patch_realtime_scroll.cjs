...
const TEMP_ = `            if (window.Capacitor && window.Capacitor.isNativePlatform()) {
...
const OLD_ = `            if (window.Capacitor && window.Capacitor.isNativePlatform()) {
                this.enableFullscreenMode();
                window.Capacitor.Plugins.App.addListener('appStateChange', ({ isActive }) => {
                    if (isActive) {
...
if (!src.includes(TEMP_)) { console.error('FIX2: appStateChange target not found'); process.exit(1); }
src = src.replace(TEMP_, OLD_);
...
const TEMP_ = `                console.log('🌈 New Magic Received!', payload);
...
const OLD_ = `                console.log('🌈 New Magic Received!', payload);
                this.state.lastDoodle = payload.new.image_data;
...
if (!src.includes(TEMP_)) { console.error('FIX1: subscribeToDoodles target not found'); process.exit(1); }
src = src.replace(TEMP_, OLD_);
...
if (!src.includes(TEMP_)) { console.error('FIX3: setView guard target not found'); process.exit(1); }
src = src.replace(TEMP_, OLD_);
...
const TEMP_ = `            if (viewName === 'history') {
...
const OLD_ = `            if (viewName === 'history') {
                if (this.state.unreadCount > 0) {
                    this.markAllRead();
                }
...
if (!src.includes(TEMP_)) { console.error('FIX7: history setView target not found'); process.exit(1); }
src = src.replace(TEMP_, OLD_);
...
const TEMP_ = `    setupHistoryInfiniteScroll() {
...
const OLD_ = `    setupHistoryInfiniteScroll() {
        const sentinel = document.getElementById('history-scroll-sentinel');
        if (!sentinel) return;
...
if (!src.includes(TEMP_)) { console.error('FIX4: setupHistoryInfiniteScroll target not found'); process.exit(1); }
src = src.replace(TEMP_, OLD_);
...
const TEMP_ = `    async loadMoreDrafts(btn) {
...
const OLD_ = `    async loadMoreDrafts(btn) {
        if (btn && typeof btn === 'object') btn.innerHTML = '<i data-lucide="loader" class="animate-spin w-3 h-3 inline"></i> Loading...';
...
if (!src.includes(TEMP_)) { console.error('FIX5: loadMoreDrafts target not found'); process.exit(1); }
src = src.replace(TEMP_, OLD_);
...
const TEMP_ = `        if (iconsNeeded && window.lucide) lucide.createIcons();
    },

    updateHistoryDOM()`;

const OLD_ = `        if (iconsNeeded && window.lucide) lucide.createIcons();
...
if (!src.includes(TEMP_)) { console.error('FIX5b: updateDraftsDOM icon section not found'); process.exit(1); }
src = src.replace(TEMP_, OLD_);
...
const TEMP_ = `    setupDraftsInfiniteScroll() {
...
const OLD_ = `    setupDraftsInfiniteScroll() {
        const sentinel = document.getElementById('drafts-scroll-sentinel');
        const scrollRow = document.getElementById('sketchbook-scroll-row');
        if (!sentinel || !scrollRow) return;
...
if (!src.includes(TEMP_)) { console.error('FIX6: setupDraftsInfiniteScroll target not found'); process.exit(1); }
src = src.replace(TEMP_, OLD_);
